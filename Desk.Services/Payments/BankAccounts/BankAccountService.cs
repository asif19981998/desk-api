...
namespace Desk.Services.Payments.BankAccounts
{
   public class BankAccountService : IBankAccountService
    {
        private readonly IEntityRepository<BankAccount> _repository;
        public BankAccountService(IEntityRepository<BankAccount> repository)
        {
        ...
        public async Task<bool> PermanentDeleteAsync(BankAccount model)
        {
        ...
        public async Task<bool> UpdateAsync(BankAccount model)
        {
            return await _repository.UpdateAsync(model);
        }

        public async Task<bool> SoftDeleteAsync(BankAccount model)
        {
        ...
        public async Task<PagedModel<BankAccount>> GetPagedListAsync(int page, int pageSize)
        {
            ...
            PagedModel<BankAccount> pagedModel = new PagedModel<BankAccount>()
            {
                Models = pagedList,
        ...
        public async Task<PagedModel<BankAccount>> GetDeletedPagedAsync(int page, int pageSize)
        {
            int resCount = TotalCount();
            var pagers = new PagedList(resCount, page, pageSize);
            int recSkip = (page - 1) * pageSize;
        ...
        public async Task<IEnumerable<BankAccount>> GetAllDeletedAsync()
        {
            return await _repository.GetAllDeletedAsync();
        }

        public async Task<bool> RestoreAsync(BankAccount model)
        {
