...
namespace Desk.Services.Employees.PersonalInformations
{
    ...
    {
        private readonly IEntityRepository<PersonalInformation> _repository;

        public PersonalInformationService(IEntityRepository<PersonalInformation> repository)
        {
            _repository = repository;
        }


        public async Task<PersonalInformation> AddAsync(PersonalInformation model)
        {
            model.CreatedOn = DateTime.Now;
            return await _repository.AddAsync(model);
        }

        public async Task<bool> PermanentDeleteAsync(PersonalInformation model)
        {
        ...
        public async Task<PersonalInformation> GetByIdAsync(long id)
        {
        ...
        public async Task<bool> UpdateAsync(PersonalInformation model)
        {
            return await _repository.UpdateAsync(model);
        }

        public async Task<bool> SoftDeleteAsync(PersonalInformation model)
        {
        ...
        public async Task<PagedModel<PersonalInformation>> GetPagedListAsync(int page, int pageSize)
        {
            ...
            PagedModel<PersonalInformation> pagedModel = new PagedModel<PersonalInformation>()
            {
                Models = pagedList,
        ...
        public async Task<PagedModel<PersonalInformation>> GetDeletedPagedAsync(int page, int pageSize)
        {
            int resCount = TotalCount();
            var pagers = new PagedList(resCount, page, pageSize);
            int recSkip = (page - 1) * pageSize;
        ...
        public async Task<IEnumerable<PersonalInformation>> GetAllDeletedAsync()
        {
            return await _repository.GetAllDeletedAsync();
        }

        public async Task<bool> RestoreAsync(PersonalInformation model)
        {
