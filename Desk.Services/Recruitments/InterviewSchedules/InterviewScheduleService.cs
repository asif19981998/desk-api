...
namespace Desk.Services.Recruitments.InterviewSchedules
{
    public class InterviewScheduleService : IInterviewScheduleService { 
        ...
        private readonly IEntityRepository<InterviewSchedule> _repository;

        public InterviewScheduleService(IEntityRepository<InterviewSchedule> repository)
        {
            _repository = repository;
        }


        public async Task<InterviewSchedule> AddAsync(InterviewSchedule model)
        {
            model.CreatedOn = DateTime.Now;
            return await _repository.AddAsync(model);
        }

        public async Task<bool> PermanentDeleteAsync(InterviewSchedule model)
        {
        ...
        public async Task<InterviewSchedule> GetByIdAsync(long id)
        {
        ...
        public async Task<bool> UpdateAsync(InterviewSchedule model)
        {
            return await _repository.UpdateAsync(model);
        }

        public async Task<bool> SoftDeleteAsync(InterviewSchedule model)
        {
        ...
        public async Task<PagedModel<InterviewSchedule>> GetPagedListAsync(int page, int pageSize)
        {
            ...
            PagedModel<InterviewSchedule> pagedModel = new PagedModel<InterviewSchedule>()
            {
                Models = pagedList,
        ...
        public async Task<PagedModel<InterviewSchedule>> GetDeletedPagedAsync(int page, int pageSize)
        {
            int resCount = TotalCount();
            var pagers = new PagedList(resCount, page, pageSize);
            int recSkip = (page - 1) * pageSize;
        ...
        public async Task<IEnumerable<InterviewSchedule>> GetAllDeletedAsync()
        {
            return await _repository.GetAllDeletedAsync();
        }

        public async Task<bool> RestoreAsync(InterviewSchedule model)
        {
