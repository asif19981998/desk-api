...
namespace Desk.Services.Core.Nationalities
{
    ...
    {
        private readonly IEntityRepository<Nationality> _repository;

        public NationalityService(IEntityRepository<Nationality> repository)
        {
            _repository = repository;
        }

        public async Task<Nationality> AddAsync(Nationality model)
        {
            model.CreatedOn = DateTime.Now;
            return await _repository.AddAsync(model);
        }

        public async Task<bool> PermanentDeleteAsync(Nationality model)
        {
        ...
        public async Task<Nationality> GetByIdAsync(long id)
        {
        ...
        public async Task<bool> UpdateAsync(Nationality model)
        {
            return await _repository.UpdateAsync(model);
        }

        public async Task<bool> SoftDeleteAsync(Nationality model)
        {
        ...
        public async Task<PagedModel<Nationality>> GetPagedListAsync(int page, int pageSize)
        {
            ...
            PagedModel<Nationality> pagedModel = new PagedModel<Nationality>()
            {
                Models = pagedList,
        ...
        public async Task<PagedModel<Nationality>> GetDeletedPagedAsync(int page, int pageSize)
        {
            int resCount = TotalCount();
            var pagers = new PagedList(resCount, page, pageSize);
            int recSkip = (page - 1) * pageSize;
        ...
        public async Task<IEnumerable<Nationality>> GetAllDeletedAsync()
        {
            return await _repository.GetAllDeletedAsync();
        }

        public async Task<bool> RestoreAsync(Nationality model)
        {
