...
namespace Desk.Services.Clients.ClientTypes
{
    ...
    {
        private readonly IEntityRepository<ClientType> _repository;

        public ClientTypeService(IEntityRepository<ClientType> repository)
        {
            _repository = repository;
        }


        public async Task<ClientType> AddAsync(ClientType model)
        {
            model.CreatedOn = DateTime.Now;
            return await _repository.AddAsync(model);
        }

        public async Task<bool> PermanentDeleteAsync(ClientType model)
        {
        ...
        public async Task<ClientType> GetByIdAsync(long id)
        {
        ...
        public async Task<bool> UpdateAsync(ClientType model)
        {
            return await _repository.UpdateAsync(model);
        }

        public async Task<bool> SoftDeleteAsync(ClientType model)
        {
        ...
        public async Task<PagedModel<ClientType>> GetPagedListAsync(int page, int pageSize)
        {
            ...
            PagedModel<ClientType> pagedModel = new PagedModel<ClientType>()
            {
                Models = pagedList,
        ...
        public async Task<PagedModel<ClientType>> GetDeletedPagedAsync(int page, int pageSize)
        {
            int resCount = TotalCount();
            var pagers = new PagedList(resCount, page, pageSize);
            int recSkip = (page - 1) * pageSize;
        ...
        public async Task<IEnumerable<ClientType>> GetAllDeletedAsync()
        {
            return await _repository.GetAllDeletedAsync();
        }

        public async Task<bool> RestoreAsync(ClientType model)
        {
