...
namespace Desk.Services.Auth.UserLogs
{
    ...
    {
        private IEntityRepository<UserLog> _repository;

        public UserLogService(IEntityRepository<UserLog> repository)
        {
            _repository = repository;
        }


        public async Task<UserLog> AddAsync(UserLog model)
        {
            model.CreatedOn = DateTime.Now;
            return await _repository.AddAsync(model);
        }

        public async Task<bool> PermanentDeleteAsync(UserLog model)
        {
        ...
        public async Task<bool> UpdateAsync(UserLog model)
        {
            return await _repository.UpdateAsync(model);
        }

        public async Task<bool> SoftDeleteAsync(UserLog model)
        {
        ...
        public async Task<PagedModel<UserLog>> GetPagedListAsync(int page, int pageSize)
        {
            ...
            PagedModel<UserLog> pagedModel = new PagedModel<UserLog>()
            {
                Models = pagedList,
        ...
        public async Task<PagedModel<UserLog>> GetDeletedPagedAsync(int page, int pageSize)
        {
            int resCount = TotalCount();
            var pagers = new PagedList(resCount, page, pageSize);
            int recSkip = (page - 1) * pageSize;
        ...
        public async Task<IEnumerable<UserLog>> GetAllDeletedAsync()
        {
            return await _repository.GetAllDeletedAsync();
        }

        public async Task<bool> RestoreAsync(UserLog model)
        {
